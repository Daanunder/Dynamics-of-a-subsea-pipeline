...
class NDOF_system(object):
    ...
    def __init__(self, R=5):
        ...
        self.kx = 0
        self.ky = self.total_kx/self.R/10
        self.kr = self.total_kx/self.R
        self.cx = 40000000/self.R
        self.element_inertia = 1/2*self.element_mass*(self.D_pipe/2)**2
        
        self.point_mass_mass = 50*10**6
        self.ky_m = self.total_kx*100000
        self.kx_m = self.total_kx*100000
        ...
        self.current_forcing = sp.symbols("F_current")
        self.current_forcing_amplitude = 2000
        self.current_forcing_freq = 1/200
        self.wave_forcing = sp.symbols("F_waves")
        ...
        self.constrained_acceleration_vector = []
    ...
    def get_element_lagrangian(self, K, P, i):
        ...
        eom1 = sp.Eq(sp.simplify(L.diff(x(t).diff(t)).diff(t) - L.diff(x(t))), self.current_forcing)
    ...
    def get_nonlin_acceleration_matrix(self):
        ...
        stiffness_vector = sp.matrices.zeros(self.matrix_size, 1)
        force_vector = sp.matrices.zeros(self.matrix_size, 1)
        rhs_vector = sp.matrices.zeros(self.matrix_size, 1)
        ...
        for m, eq in enumerate(eoms):
            ...
            gravity_present = eq.free_symbols.intersection([self.g]) == set([self.g])
            mass_symbol = list(eq.free_symbols.intersection(all_consts))[0]

            ## STIFFNESS VECTOR
            # sub all accelerations to zero
            temp_eq = eq.subs({(acc, 0) for acc in self.all_accelerations})
            ...
            if not temp_eq == True and not temp_eq == False:
                stiffness_vector[m] = temp_eq.lhs
            
            ## FORCING
            if eq.rhs and gravity_present:
                force_vector[m] = eq.rhs - self.g*mass_symbol
            elif eq.rhs:
                force_vector[m] = eq.rhs
            elif gravity_present:
                force_vector[m] = -self.g*mass_symbol

            rhs_vector = force_vector - stiffness_vector

        self.mass_matrix = mass_matrix
        self.stiffness_vector = stiffness_vector
        self.force_vector = force_vector
        self.rhs_vector = rhs_vector
        self.acceleration_vector = mass_matrix.inv() * rhs_vector

        return self.acceleration_vector

    def get_linear_acceleration_vector(self):
        eoms = self.sub_eoms_with_symbols()
        # eps_dict
        eps_dict = dict()
        for acc in self.all_accelerations:
            eps_dict[acc] = sp.symbols("e_"+str(acc))
        for vel in self.all_velocities:
            eps_dict[vel] = sp.symbols("e_"+str(vel))
        for pos in self.all_positions:
            eps_dict[pos] = sp.symbols("e_"+str(pos))

        lin_origin = dict()
        for acc in self.all_accelerations:
            lin_origin[acc] = sp.symbols(str(acc)+"_0")
        for vel in self.all_velocities:
            lin_origin[vel] = sp.symbols(str(vel)+"_0")
        for pos in self.all_positions:
    ...
    def substitute_system_constraints(self):
        if not self.acceleration_vector:
            self.get_nonlin_acceleration_matrix()

        all_constraints = self.setup_system_constraints()
        self.constrained_acceleration_vector = sp.matrices.zeros(self.matrix_size, 1)
        
        for m in range(self.matrix_size):
            eq = self.acceleration_vector[m]
            while any(str(v).startswith("x_") or str(v).startswith("y_") for v in eq.free_symbols):
                eq = eq.subs(all_constraints)

            self.constrained_acceleration_vector[m] = eq
        
            for s in eq.free_symbols:
                if s != t:
                    self.current_state[s] = None
    
        return self.constrained_acceleration_vector

    ## FORCING FUNCTIONS
    def current_forcing_function(self, time, a=None, omega=None):
        if not a:
            a = self.current_forcing_amplitude
        if not omega:
            omega = self.current_forcing_freq 
    ...
    def compute_accelerations(self, qn, time):
        print(time)
        for i,d in self.elements.items():
            j = i - 1
        ...
        for m in range(self.matrix_size):
            eq = self.constrained_acceleration_vector[m]
            f_res = self.current_forcing_function(time)
            f_sym = self.current_forcing
            result = eq.subs([(f_sym, f_res)])
            result = result.evalf(subs=self.current_state)
    ...
    def qdot(self, time, qn):
        ...
        acc_vector = self.compute_accelerations(qn, time)
    ...
    def ivp_solver(self, t_eval=None):
        ...
        if not t_eval:
            ...
            sol = solve_ivp(self.qdot, (self.t_start, self.t_end), q0, t_eval=t_eval, atol=self.eps_abs, rtol=self.eps_rel)
    ...
    def get_forcing(self, Hm0, Tp, tspan, plot=False):
        F, E, f = get_forcing(Hm0, Tp, tspan)
        if plot:
            plt.plot(tspan, F)
            plt.title(f"Forcing time series with Hm0: {Hm0}, Tp: {Tp}")
            ...
            plt.title(f"JONSWAP spectrum with Hm0: {Hm0}, Tp: {Tp}")
