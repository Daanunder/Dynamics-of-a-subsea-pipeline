...
class NDOF_system(object):
    ...
    def get_element_lagrangian(self, K, P, i):
        ...
        #eom1 = sp.Eq(sp.simplify(L.diff(x(t).diff(t)).diff(t) - L.diff(x(t))), self.current_forcing(t,self.y))
    ...
    def sub_eoms_with_symbols(self, print_latex=False):
        ...
        for j,eq in enumerate(self.EOMS):
            ...
            if j == 0:
                double_derivative_symbols.extend([xm_dd, ym_dd])
            ...
            for i,d in self.elements.items():
                ...
                for v in d["vars"]:
                    ...
                    if j == 0:
                        double_derivative_symbols.append(vdd)
        ...
        if print_latex:
            ...
            return new_EOMS, double_derivative_symbols


    def get_nonlin_acceleration_matrix(self):
        eoms, dd_symbols = self.sub_eoms_with_symbols()
        matrix = []
        for j, a in enumerate(dd_symbols):

            ## To get rid of the acceleration coupling we set the orthogonal acceleration to zero for x and y
            if str(a).startswith("x"):
                eq = eoms[j].subs([(dd_symbols[j+1], 0)])
            elif str(a).startswith("y"):
                eq = eoms[j].subs([(dd_symbols[j-1], 0)])
            else:
                eq = eoms[j]

            sol = sp.solveset(eq,a)
            matrix.append([a, sol])
        ...
        matrix_p1 = matrix[2:]
        matrix_p2 = matrix[:2]
        matrix = matrix_p1 + matrix_p2

        # save and return
        self.acceleration_matrix = matrix
    ...
    def construct_q_from_theta(self):
        ...
        q[self.R*3, 0] = cmx.evalf(subs=self.current_state)
        q[self.R*3+1, 0] = cmy.evalf(subs=self.current_state)
        q[self.R*3, 1] = cmxd.evalf(subs=self.current_state)
        q[self.R*3+1, 1] = cmyd.evalf(subs=self.current_state)
        return q
    ...
    def compute_accelerations(self, qn, t):
        ...
        for i,d in self.elements.items():
            ...
            self.current_state[theta_sym] = qn[3*i-1, 0]
            self.current_state[theta_dot_sym] = qn[3*i-1, 1]

        acc = []
        for a, eq in self.constrained_acceleration_matrix:
            result = eq.evalf(subs=self.current_state)
            if type(result) == sp.core.numbers.NaN:
                print(a, result)
                self.append_equation_to_latex(eq)
                result = 0
    ...
    def ivp_solver(self):
        ...
        # solve ivp
        sol = solve_ivp(self.qdot, (self.t_start, self.t_end), q0)
        self.rk45_sol = sol
